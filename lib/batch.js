...
import {parseFilterValues} from './search.js'

function normalizeFilters(params, availableFilters) {
  ...
  for (const [filterName, filterConfig] of Object.entries(availableFilters)) {
    if (params.filters[filterName]) {
      const values = parseFilterValues(filterName, params.filters[filterName], filterConfig.maxValues)
      if (values.length > 0) {
        normalizedFilters[filterName] = values
      }
    }
  }
...
export function batch({cluster, filters: availableFilters}) {
  return async (req, res) => {
    ...
    const results = await Promise.all(requests.map(async r => {
      ...
      try {
        const mergedParams = {...globalParams, ...params}
        const rParams = normalizeFilters(mergedParams, availableFilters)
        const operationResult = await cluster[operation](rParams, {signal: ac.signal})
